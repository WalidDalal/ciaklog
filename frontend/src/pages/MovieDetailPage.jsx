        ...
        <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', backgroundColor: 'var(--bg-card)', border: '1px solid #333', borderRadius: '16px', padding: '32px', width: '100%', maxWidth: '460px', zIndex: 600 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '20px' }}>
            {REPORT_CATEGORIES.map(c => (
                <button key={c.value} onClick={() => setCategory(c.value)} style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', padding: '12px 16px', borderRadius: '8px', textAlign: 'left', border: `1px solid ${category === c.value ? 'var(--accent)' : 'var(--border-soft)'}`, backgroundColor: category === c.value ? '#1a0a0a' : 'var(--bg-card)', cursor: 'pointer', width: '100%' }}>
...
// Selettore stagione — appare inline sotto i bottoni stato quando si sceglie WATCHING su una serie
function SeasonSelector({ value, onChange, onConfirm, onCancel }) {
  return (
      <div style={{ marginTop: '12px', padding: '16px', backgroundColor: 'var(--bg-card)', border: '1px solid var(--accent)', borderRadius: '10px', display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
        <span style={{ color: 'var(--text-muted)', fontSize: '13px' }}>Stagione in visione:</span>
        <select value={value || ''} onChange={e => onChange(Number(e.target.value) || null)}
                style={{ padding: '6px 10px', backgroundColor: 'var(--bg-hover)', border: '1px solid #444', borderRadius: '6px', color: 'var(--text)', fontSize: '14px', cursor: 'pointer' }}>
          <option value="">—</option>
          {Array.from({ length: 20 }, (_, i) => i + 1).map(n => (
              <option key={n} value={n}>Stagione {n}</option>
          ))}
        </select>
        <button onClick={onConfirm}
                style={{ padding: '6px 16px', backgroundColor: 'var(--accent)', border: 'none', borderRadius: '6px', color: 'var(--text)', fontSize: '13px', fontWeight: '600', cursor: 'pointer' }}>
          Conferma
        </button>
        <button onClick={onCancel}
                style={{ padding: '6px 12px', backgroundColor: 'transparent', border: '1px solid #444', borderRadius: '6px', color: 'var(--text-muted)', fontSize: '13px', cursor: 'pointer' }}>
          Annulla
        </button>
      </div>
  )
...
function MovieDetailPage() {
  ...
  // Selettore stagione — appare solo quando si clicca WATCHING su una serie
  const [seasonPickerPending, setSeasonPickerPending] = useState(false)
  ...
  const handleAddToLibrary = async (status, season = null) => {
    ...
    // Se è una serie e si sceglie WATCHING → mostra selettore stagione prima di procedere
    if (isTV && status === 'WATCHING' && !seasonPickerPending) {
      setPendingStatus(status)
      setSeasonPickerPending(true)
      setSelectedSeason(watchEntry?.currentSeason || null)
      return
    }

    setLibraryLoading(true); setLibraryError('')
    setSeasonPickerPending(false); setPendingStatus(null)
    try {
      if (watchEntry) {
        const res = await api.put(`/library/${watchEntry.id}`, null, { params: { status } })
        setWatchEntry({ ...res.data, currentSeason: season ?? res.data.currentSeason })
        // Aggiorna stagione separatamente se cambiata
        if (season !== null) {
          const res2 = await api.put(`/library/${watchEntry.id}/season`, null, { params: { season } })
          setWatchEntry(res2.data)
        }
      } else {
          ...
          <div style={{ flex: 1 }}>
            ...
            {!isAdmin && (
                <>
                  ...
                  {/* Selettore stagione — appare solo per serie TV quando si sceglie WATCHING */}
                  {seasonPickerPending && isTV && (
                      <SeasonSelector
                          value={selectedSeason}
                          onChange={setSelectedSeason}
                          onConfirm={() => handleAddToLibrary(pendingStatus, selectedSeason)}
                          onCancel={() => { setSeasonPickerPending(false); setPendingStatus(null); setSelectedSeason(null) }}
                      />
                  )}

                  {/* Stagione attuale se in visione */}
                  {watchEntry?.status === 'WATCHING' && watchEntry?.currentSeason && (
                      <p style={{ color: 'var(--text-muted)', fontSize: '13px', marginTop: '6px' }}>
                        📺 Stagione {watchEntry.currentSeason} in visione
                      </p>
                  )}
                ...
                <div style={{ marginTop: '28px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                    {detail.cast.slice(0, 8).map((actor, i) => {
                      ...
                      const photo = typeof actor === 'object' ? actor.photoPath : null
                      return (
                          <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px', width: '72px' }}>
                            {photo
                                ...
                                : <div style={{ width: '56px', height: '56px', borderRadius: '50%', backgroundColor: 'var(--bg-hover)', border: '2px solid #333', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '20px' }}>👤</div>
                            }
                            <span style={{ color: 'var(--text-muted)', fontSize: '11px', textAlign: 'center', lineHeight: 1.3, wordBreak: 'break-word' }}>{name}</span>
                          </div>
                      )
                    })}
                  </div>
